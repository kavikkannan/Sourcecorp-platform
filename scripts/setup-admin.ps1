# SourceCorp Platform - Setup Admin User Script (PowerShell)
# This script helps create the first admin user with all permissions

Write-Host "========================================"
Write-Host "SourceCorp Platform - Admin Setup"
Write-Host "========================================"
Write-Host ""
...
$ADMIN_FIRST_NAME = Read-Host "Admin First Name"
$ADMIN_LAST_NAME = Read-Host "Admin Last Name"

Write-Host ""
Write-Host "Generating password hash..."

# Generate password hash
$PASSWORD_HASH = docker-compose exec -T backend node -e "const bcrypt = require('bcryptjs'); console.log(bcrypt.hashSync('$ADMIN_PASSWORD_PLAIN', 10));" 2>&1 | Select-Object -Last 1
...
# Execute SQL
$SQL_SCRIPT | docker-compose exec -T postgres psql -U sourcecorp_user -d sourcecorp
